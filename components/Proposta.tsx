  ...
  return (
    <div className="bg-[#38457a] w-full flex items-center justify-between rounded-md px-3 py-2">
      ...
      <div className="text-right">
        ...
        <p className="text-white text-md font-normal">Enviada por: {prop.nomeVendedor}</p>
      </div>
      <div className="flex gap-3">
        <button className="w-fit font-semibold border border-[#ffffffa6] hover:bg-[#ffffff16] transition-all text-white rounded-md p-2" onClick={() => window.open(prop.link_pdf, '_blank')}>Ver</button>
        <button className="w-fit font-semibold border bg-[#ffffffa6] text-[#38457a] transition-all hover:opacity-60 rounded-md p-2" onClick={downloadPdf}>Baixar</button>
      </div>
    </div>
  )
}

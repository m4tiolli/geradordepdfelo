...
function CriarSenha() {
  ...
  return (
    <div className="bg-azul rounded-md flex flex-col items-center gap-4 p-4 relative z-90">
        ...
        >
          <input
            className={`bg-[#ffffff0e] border min-w-fit w-full transition-all ${
              value.value !== '' ? 'border-[#ffffff]' : 'border-[#ffffff27]'
          ...
          {value.type === 'text' ? (
            <FaEyeSlash
              onClick={() => ChangeVisibility(key as keyof PasswordFields)}
              className="text-white cursor-pointer font-2xl absolute right-2"
            />
          ) : (
            <FaEye
              onClick={() => ChangeVisibility(key as keyof PasswordFields)}
      ...
      <button
        ...
        className="text-azul bg-white py-2 px-4 rounded-md w-full transition-all hover:opacity-60 cursor-pointer font-semibold grid place-items-center"
      >

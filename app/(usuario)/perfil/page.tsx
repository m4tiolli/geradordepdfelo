...
function Perfil() {
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 py-4 rounded-md min-w-[90%] xl:min-w-[30vw] h-fit gap-4 bg-[#38457a] z-10">
      ...
      {inputs.slice(0, 1).map(({ name, placeholder, value, onChange }, index) => (
      ...
      <div className="-my-1 w-full">
        ...
        <select
          ...
          onChange={onChange}
          disabled={!editing}
          className={`bg-[#ffffff0e] border transition-all ${usuario.departamento !== ""
            ? "border-[#ffffff]"
            : "border-[#ffffff27]"
      ...
      {inputs.slice(1, 6).map(({ name, placeholder, value, onChange }, index) => (
        <div className="-my-1 w-full" key={index++}>
          <p className="font-medium text-white text-md">{placeholder}</p>
          <Input
            name={name}
      ...
      <div className="flex justify-between w-full w items-center">
        ...
        <button
          className={`text-green-800 font-semibold bg-white px-4 py-2 rounded-md w-[45%] transition-all hover:opacity-70 ${editing
